...
public class NoteNew : MonoBehaviour
{
    ...
    public void SetTail(double deltaTime)
    {
        ...
        m_TailLength = speed * (float)deltaTime / 0.005f;
        m_DeltaTime = deltaTime;

        transform.GetChild(0).localPosition = new Vector3(0.0f, 0.0f, m_TailLength);
        transform.GetChild(0).localScale = new Vector3(0.2f, 0.5f, m_TailLength);
    }
    ...
    public void UpdateTail(double deltaTime)
    {
        ...
        transform.GetChild(0).localScale = new Vector3(0.2f, 0.5f, length);
        m_IsPressing = true;
    }
