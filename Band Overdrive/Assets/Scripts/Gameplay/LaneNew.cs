    ...
    void Update()
    {
        ...
        if (m_SpawnIndex < m_Notes.Count)
        {
            ...
            if (m_Track.GetTimeElapsed() >= currNoteSpawnTime)
            {
                ...
                noteObject.AddComponent<NoteNew>();
                if (currNote.hasTail)
                    noteObject.GetComponent<NoteNew>().SetTail(currNote.deltaTime);

                noteObject.GetComponent<NoteNew>().SetSolo(currNote.isSolo);
                noteObject.GetComponent<NoteNew>().SetOverDrive(currNote.isOverDrive);
                noteObject.GetComponent<NoteNew>().SetHopo(currNote.isHopo);
                noteObject.GetComponent<NoteNew>().SetCymbal(currNote.isCymbal);

                noteObject.GetComponent<NoteNew>().Spawn(m_Track.GetTimeElapsed());

                m_CurrNotes.Add(noteObject.GetComponent<NoteNew>());
                m_SpawnIndex++;
            }
        }
